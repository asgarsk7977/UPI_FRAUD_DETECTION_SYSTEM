...
if "quiz_qno" not in st.session_state:
    st.session_state.quiz_qno = 0
...
if not st.session_state.admin_logged_in:
    with st.sidebar.expander("Admin Login"):
        ...
        if st.button("Login"):
            if user_input == "asgar" and pwd_input == "asgar123":
...
if st.session_state.admin_logged_in:
    nav_options = ["Admin Dashboard","Manage Blacklist", "Create Security Quiz"]
    st.sidebar.title("👨‍💼 Admin Panel")
else:
    nav_options = ["Single Detection", "Bulk Upload", "Analytics Dashboard", "Fraud Helpline", "Verified Fraud UPIs","Play Quiz"]
...
if st.session_state.admin_logged_in:
    ...
    if menu == "Create Security Quiz":
        st.title("🧠 Admin Quiz Manager")
        ...
        if st.button("Save Quiz"):
            new = pd.DataFrame([[q,a,b,c,d,correct]],
                columns=["QUESTION","OPTION_A","OPTION_B","OPTION_C","OPTION_D","CORRECT"])

            old = pd.read_csv(QUIZ_FILE)
            pd.concat([old,new]).to_csv(QUIZ_FILE,index=False)

            st.success("Question Added Successfully")

        st.divider()
        st.subheader("Current Questions")
        st.dataframe(pd.read_csv(QUIZ_FILE),use_container_width=True)
...
# ======================= END OF PART 1 =======================
    ...
    "DATETIME": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ...
        st.title("🎮 SecurePay Awareness Quiz")

        quiz = pd.read_csv(QUIZ_FILE)

        if len(quiz) == 0:
            st.info("No quiz available yet")
        else:

            if st.session_state.quiz_qno < len(quiz):

                row = quiz.iloc[st.session_state.quiz_qno]

                st.write(f"### Q{st.session_state.quiz_qno+1}. {row['QUESTION']}")
                ...
                if st.button("Next"):
                    result_entry = {
                         "Q No": st.session_state.quiz_qno + 1,
                         "Question": row['QUESTION'],
                         "Your Selection": options[ans],
                         "Correct Answer": options[row["CORRECT"]],
                           "Result": "✅ Correct" if ans == row["CORRECT"] else "❌ Incorrect"
                           }
                    st.session_state.quiz_results.append(result_entry)
                    if ans == row["CORRECT"]:
                        st.session_state.quiz_score += 1

                    st.session_state.quiz_qno += 1
                    st.rerun()
                ...
                st.success(f"Final Score : {score}/{total}  ({percent}%)")
                st.subheader("📝 Your Quiz Summary")
                res_df = pd.DataFrame(st.session_state.quiz_results)
                if not res_df.empty:
                    def style_quiz_rows(row):
                        color = 'background-color: #f8d7da' if 'Incorrect' in row['Result'] else 'background-color: #d4edda'
                        return [color] * len(row)
                    styled_df = res_df.style.apply(style_quiz_rows, axis=1)
                    st.dataframe(styled_df, use_container_width=True, hide_index=True)
                st.divider()
                
                if percent == 100:
                    st.balloons()
                    st.markdown("### 🏆 Outstanding! You're a SecurePay Safety Champion!")
                    st.info("You have excellent awareness of UPI fraud risks. Keep protecting yourself and others.")
                elif percent >= 70:
                    st.markdown("### 🌟 Great Job!")
                    st.info("You’re well informed about digital payment safety. A little more practice and you'll be perfect.")
                elif percent >= 40:
                    st.markdown("### 👍 Good Effort!")
                    st.info("You’re learning — stay alert, avoid unknown UPI requests, and keep improving.")
                else:
                    st.markdown("### 💡 Keep Going!")
                    st.warning("Every attempt makes you smarter. Review fraud signs and try again.")
                st.markdown("---")
                st.markdown("### 🔐 Remember: Never share OTP, PIN, or click unknown payment links.")
